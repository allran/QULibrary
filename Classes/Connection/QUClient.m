...
-(void)urlType:(kURLLinkType)type group:(NSString *)key path:(NSString *)URLString parameters:(id)parameters call:(void (^)(id obj, RespInfo* info))callback
{
    ...
    if (type == kURLLink_GET) {
        operation = [self GET:URLString parameters:parameters success:^(AFHTTPRequestOperation *operation, id responseObject) {
            [self encodeWithObj:responseObject call:^(id obj, RespInfo *info) {
                [self removeConnection:operation group:key];
                callback(obj, info);
            }];
        } failure:^(AFHTTPRequestOperation *operation, NSError *error) {
            [self removeConnection:operation group:key];
            callback(nil, [RespInfo infoWithError:error]);
        }];
...
{
    [self urlType:kURLLink_GET group:key path:URLString parameters:parameters call:^(id obj, RespInfo *info) {
        callback(obj, info);
    }];
}
...
{
    [self urlType:kURLLink_POST group:key path:URLString parameters:parameters  call:^(id obj, RespInfo *info) {
        callback(obj, info);
    }];
}
...
-(void)tableListGroup:(NSString *)key path:(NSString *)URLString parameters:(NSMutableDictionary *)parameters index:(int)index subClass:(Class)aClass call:(TableBlock)callback
{
    ...
    [self postWithGroup:key path:URLString parameters:parameters call:^(id obj, RespInfo *info) {
        [self encodeWithObj:obj info:info jsonTotalCountKey:@"totalcount" jsonListKey:@"list" subClass:aClass tableCall:^(int totalCount, NSArray *tableArr, RespInfo *info) {
